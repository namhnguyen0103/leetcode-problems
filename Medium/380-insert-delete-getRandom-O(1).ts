...
class RandomizedSet {
    randomSet: { [value: number] : number | undefined };
    values: number[];
    ...
    insert(val: number): boolean {
        ...
        this.randomSet[val] = this.values.length - 1;
        return true;
    }
    ...
    remove(val: number): boolean {
        if (this.randomSet[val] !== undefined) {
            ...
            const newIdx = this.randomSet[val];
            this.values[newIdx] = lastVal;
            this.randomSet[lastVal] = newIdx;
            this.values.pop();
    ...
    getRandom(): number {
        ...
        return this.values[Math.floor(Math.random()*length)];
    }
}
